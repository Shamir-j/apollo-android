...
class ServiceTests {
  ...
  fun `customScalarsMapping can be applied from a service block`() {
    ...
    """.trimIndent()) { dir ->
      TestUtils.executeTask("generateApolloSources", dir)
      TestUtils.assertFileContains(dir, "api/com/example/type/CustomScalars.kt", "\"java.util.Date\"")
    }
  }
  ...
  fun `rootPackageName works as expected`() {
    ...
    """.trimIndent()) { dir ->
      ...
      assertTrue(dir.generatedChild("service/com/starwars/com/example/type/CustomScalars.kt").isFile)
  ...
  fun `sourceFolder can be changed`() {
    ...
    """.trimIndent()) { dir ->
      ...
      assertTrue(dir.generatedChild("starwars/type/CustomScalars.kt").isFile)
